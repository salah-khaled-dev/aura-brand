...
export default function ShippingCostsPage() {
  ...
  const t = adminAr.business.shippingCosts;
  ...
  const filteredData = useMemo(() => data.filter(e => e.category === 'shipping'), [data]);
  ...
  const handleSave = async (expenseData: Omit<Expense, 'id'>) => {
    if (editingExpense) {
      ...
      await businessService.createExpense({ ...expenseData, category: 'shipping' });
      toast.success(adminAr.toasts.dataSaved);
    }
    fetchData();
  };
