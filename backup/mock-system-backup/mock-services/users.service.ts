...
export const UsersService = {
  ...
  async createStaff(
    ...
    // Create the real Supabase Auth user + profile first, so this staff member
    // can actually sign in. The staff directory / role matrix below stays mock
    // (no `roles` table in Supabase yet) but now keys off the real auth user id.
    const res = await fetch('/api/admin/staff', {
      ...
      body: JSON.stringify({
        ...
        phone: data.phone,
        staff_role_key: role?.nameKey ?? null,
